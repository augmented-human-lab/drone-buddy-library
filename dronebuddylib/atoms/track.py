...
sys.path.append(str(Path(__file__).resolve().parent) + '\\resources\\tracking\\pytracking')
from resources.tracking.pytracking.pytracking.evaluation import Tracker
import cv2
...
def init(img, box, tracker: TrackEngine):
  """
  Initialize the TrackEngine with the target to be tracked.
...
def get_bounding_box(img, tracker: TrackEngine):
  """
