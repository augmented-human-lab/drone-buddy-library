#!/usr/bin/env python3
"""
Windows-specific real-time drone controller for 2D hierarchical waypoint mapping.

Provides real-time manual control of Tello drones for creating 2D hierarchical waypoint maps
using Windows-compatible input handling (msvcrt). Records all movements, tracks Super Waypoints
...
class RealTimeDroneControllerWindows:
    """Windows real-time controller for 2D hierarchical drone waypoint mapping."""
    ...
    def __init__(self, waypoint_dir: str, movement_speed: int, rotation_speed: int, nav_speed: int = 30, vertical_factor: float = 1.5):
        ...
        logger.log_info('RealTimeDroneControllerWindows', f'Initialized 2D mapping controller')
                ...
                for part in attitude_parts:
                    if part.strip() and 'yaw:' in part:
                        try:
                            ...
                            logger.log_warning('RealTimeDroneControllerWindows', f'Failed to parse yaw: {e}')
                            continue
            return yaw
        except Exception as e:
            logger.log_warning('RealTimeDroneControllerWindows', f'Attitude query failed: {e}')
    ...
    def stop_movement(self, drone_instance=None):
        ...
        if not self.add_movement:
            ...
            try:
                ...
                logger.log_error('RealTimeDroneControllerWindows', f'Error stopping rotation: {e}')
            self.current_movement = None
            return
        ...
        logger.log_info('RealTimeDroneControllerWindows', 
    ...
    def _execute_return_to_super_waypoint(self, movements: List[Movement], drone_instance=None) -> bool:
        ...
        if not movements:
            logger.log_info('RealTimeDroneControllerWindows', 'No movements to reverse, already at Super Waypoint')
            return True
        ...
        try:
            for i, movement in enumerate(reversed_movements, 1):
                ...
                try:
                    battery_str = drone_instance.send_command_with_return("battery?", timeout=3)
                    logger.log_debug('RealTimeDroneControllerWindows', 'Checking battery status')
                    battery = int(battery_str)
                    if battery < 20:
                        logger.log_warning('RealTimeDroneControllerWindows', f'Low battery detected: {battery}%')
                        if battery < 10:
                            logger.log_error('RealTimeDroneControllerWindows', 
                                f'CRITICAL: Battery too low ({battery}%), aborting return')
                            return False
                except:
                    pass
                
                logger.log_debug('RealTimeDroneControllerWindows', 
            ...
            logger.log_success('RealTimeDroneControllerWindows', 'Successfully returned to Super Waypoint')
            print("✅ Returned to Super Waypoint!")
            return True
            
        except Exception as e:
            logger.log_error('RealTimeDroneControllerWindows', f'Error during return: {e}')
            drone_instance.send_rc_control(0, 0, 0, 0)
            return False
    ...
    def mark_inner_waypoint(self, name: str = None, drone_instance=None) -> Optional[InnerWaypoint]:
        ...
        if success:
            ...
            logger.log_warning('RealTimeDroneControllerWindows', 
            ...
            while self.video_running and self.frame_read:
                try:
                    ...
                    if frame is not None and frame.size > 0:
                        ...
                        # Show current Super Waypoint info (within overlay area)
                        ...
                        # Pin window to topmost on first display only
                        if first_time:
                            hwnd = win32gui.FindWindow(None, window_name)
                            win32gui.SetWindowPos(hwnd, win32con.HWND_TOPMOST, 0, 0, 0, 0,
                                                win32con.SWP_NOMOVE | win32con.SWP_NOSIZE)
                            first_time = False  # Subsequent frames will not re-pin the window
    ...
    def get_key(self):
        """Get single keypress with timeout - Windows implementation."""
        start_time = time.time()
        timeout = 0.2
                ...
                if current_time - last_battery_check > 5:
                    try:
                        ...
                        if battery < 20:
                            ...
                            if battery < 10:
                                logger.log_error('RealTimeDroneControllerWindows', 'CRITICAL: Battery too low')
                                break
                        last_battery_check = current_time
                    except Exception as e:
                        logger.log_error('RealTimeDroneControllerWindows', f'Battery check error: {e}')

                key = self.get_key()
                
                if key:
                    logger.log_debug('RealTimeDroneControllerWindows', f'Key: {key}')
    ...
    def run(self, drone_instance=None) -> list:
        """Main control loop for 2D mapping."""
        print("\n" + "=" * 60)
        ...
        logger.log_info('RealTimeDroneControllerWindows', 'First Super Waypoint marked: START')
