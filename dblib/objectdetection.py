...
def detect_common_object_labels(image, confidence=0.5, nms_thresh=0.3, model='yolov4', enable_gpu=False,
                                detection_type: ObjectDetectionReturnTypes = ObjectDetectionReturnTypes.ALL):
    bbox, label, conf = detect_common_objects(image, confidence, nms_thresh, model, enable_gpu)

    if detection_type == ObjectDetectionReturnTypes.ALL:
        return bbox, label, conf
    elif detection_type == ObjectDetectionReturnTypes.BBOX:
        return bbox
    elif detection_type == ObjectDetectionReturnTypes.LABELS:
        return label
    elif detection_type == ObjectDetectionReturnTypes.CONF:
        return conf
    else:
        return bbox, label, conf
